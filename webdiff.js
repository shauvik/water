...
var win1=selenium.browserbot.getCurrentWindow();
win1.document.expando = true;
var bcc = {
  version:[0.1,20091109],
  init:function(){
    var dtObj1=levelorderTraversal(win1.document.body);
...
function getElementDOMPath(elt)
{
     var path = "";
     for (; elt && elt.nodeType == 1; elt = elt.parentNode)
     {
...
function getAbsolutePos(element) {
	...
	if (element !== null) {
		...
		while (offsetParent != null) {
			...
			if (offsetParent != win1.document.body && offsetParent != win1.document.documentElement) {
...
function Node(arktos,domid,nodeName, id, domNumber,nodeType,x0,y0,x1,y1,clickable,xpath,zindex,visible,domName,linkText,classes) {
...
Node.prototype.toString = function() {
    //return "{"this.domid + " " + this.nodeName+"("+this.getChildrenNos()+") "++"}";
	return "("+encode64(this.domid)+")+-h+-(" +encode64(this.nodeName)+")+-h+-("+encode64(this.id)+")+-h+-("+encode64(this.nodeType)+")+-h+-("+encode64(this.getChildrenNos())+")+-h+-("+encode64(this.x0)+")+-h+-("+encode64(this.y0)+")+-h+-("+encode64(this.x1)+")+-h+-("+encode64(this.y1)+")+-h+-("+encode64(this.clickable)+")+-h+-("+encode64(this.parent)+")+-h+-("+encode64(this.xpath)+")+-h+-("+encode64(this.zindex)+")+-h+-("+encode64(this.visible)+")+-h+-("+encode64(this.childHash)+")+-h+-("+encode64(this.domName)+")+-h+-("+encode64(this.linkText)+")+-h+-("+encode64(this.classes)+")"+"\n";
};
...
Node.prototype.getChildrenNos = function() {
	var allRetChildren=new Array();
   for(var i=0;i<this.children.length;i++){
		allRetChildren[i]=this.children[i].id;
   }
      return allRetChildren;
};
...
while(q1.length) {
 ...
 for(var i=0; i<q1.length; i++) {
   if(q1[i].parentNode.visId==undefined && q1[i].parentNode.nodeName=="HTML"){
   	...
   	allNodes.push(new Node(q1[i].parentNode.getAttribute('arktos'),"notPresent",q1[i].parentNode.nodeName,q1[i].parentNode.visId,-1,q1[i].parentNode.nodeType,returnStuff[0],returnStuff[1],(returnStuff[0]+returnStuff[2]),(returnStuff[1]+returnStuff[3]),0,getElementXPath(q1[i]),11111111,1,getDomName(q1[i]),getLinkText(q1[i]),getClasses(q1[i])));
   }
    if(q1[i].parentNode.visId==undefined && q1[i].parentNode.nodeName=="BODY"){
    returnStuff=getDOMCoords(q1[i].parentNode);
  
   	q1[i].parentNode.visId=1;
   	var n=new Node(q1[i].parentNode.getAttribute('arktos'),"notPresent",q1[i].parentNode.nodeName,q1[i].parentNode.visId,-1,q1[i].parentNode.nodeType,returnStuff[0],returnStuff[1],(returnStuff[0]+returnStuff[2]),(returnStuff[1]+returnStuff[3]),0,getElementXPath(q1[i]),11111111,1,getDomName(q1[i]),getLinkText(q1[i]),getClasses(q1[i]));
   	n.addParent(0);
...
//	   alert("XX after visId"+q1[i].nodeName);
  ...
  q1[i].visId=idNo;
  //alert(q1[i]+" "+q1[i].length);
  
  try{
  q1[i].setAttribute('nodeid', idNo);
  //alert(idNo);
  }catch(e){
	asdfghj=1;
	//alert(e+" "+q1[i].nodeName);
  }
  var clickVar=1;
 ...
 returnStuff=getDOMCoords(q1[i]);
	...
	try{
  		var n=new Node(q1[i].getAttribute('arktos'),q1[i].id,q1[i].nodeName,q1[i].visId,-1,q1[i].nodeType, returnStuff[0],returnStuff[1],(returnStuff[0]+returnStuff[2]),(returnStuff[1]+returnStuff[3]),clickVar,getElementXPath(q1[i]),zIndexVal,visiblityVar,getDomName(q1[i]),getLinkText(q1[i]),getClasses(q1[i]));
	}catch(e){
		// For handling comment tags that do not have an arktos id
  		var n=new Node(0,q1[i].id,q1[i].nodeName,q1[i].visId,-1,q1[i].nodeType, returnStuff[0],returnStuff[1],(returnStuff[0]+returnStuff[2]),(returnStuff[1]+returnStuff[3]),clickVar,getElementXPath(q1[i]),zIndexVal,visiblityVar,getDomName(q1[i]),getLinkText(q1[i]),getClasses(q1[i]));
	}
...
function getStyle(oElm, strCssRule){
	var strValue = "";
	if(win1.document.defaultView && win1.document.defaultView.getComputedStyle && oElm.nodeName!="#comment"){
		strValue = win1.document.defaultView.getComputedStyle(oElm, "").getPropertyValue(strCssRule);
		}
...
function getLinkText(elt){
	strLinkText="";
	if(elt.nodeName=="A"){
		strLinkText=elt.text;
	}
	return strLinkText;
}
...
function getClasses(elt){
	strName="";
	if(elt.className!=undefined){
		strName=elt.className;
	}
	return strName;
}
